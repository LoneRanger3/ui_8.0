 ...
 * @LastEditTime: 2022-09-30 16:29:54
 */
...
int main(int argv, char **argc)
{
    ...
    FileManagerInit();
    CrLogOpen(GetUserDataLogDirPtah(), (char *)"audio-server");
    pthread_t*serverMessagehandle = ServerMessageInit(NULL);

    if (NULL != serverMessagehandle) {
        pthread_join(*serverMessagehandle, NULL);
        CrLogE("Release serverMessagehandle thread resources!");
    } else {
        CrLogE("ServerMessageInit failed!");
    }
