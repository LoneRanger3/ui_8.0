 ...
 * @LastEditTime: 2023-03-18 14:49:56
 */
...
{
    ui_UpdateVersion = lv_obj_create(ui_Settings);
    lv_obj_clear_flag(ui_UpdateVersion, LV_OBJ_FLAG_SCROLLABLE);      /// Flags
    lv_obj_set_width(ui_UpdateVersion, 800);
    lv_obj_set_height(ui_UpdateVersion, 480);
    lv_obj_set_x(ui_UpdateVersion, 0);
    lv_obj_set_y(ui_UpdateVersion, 0);
    ...
    lv_obj_set_height(ui_UpdateVersionPanel1, 442);
    ...
    lv_obj_set_y(ui_UpdateVersionBtn1, 371);
    ...
    lv_obj_set_y(ui_UpdateVersionBtn2, 371);
...
void refresh_updatepage(int flag, char * current_version, char * new_version, char * version_size, char * content, int result)
{
    ...
    wifi_wait_animmotion(false);//默认取消一次等待弹窗
    if(result)
    {
