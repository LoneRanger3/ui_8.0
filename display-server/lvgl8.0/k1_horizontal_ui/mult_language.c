 ...
 * @LastEditTime: 2023-03-17 20:33:31
 */
...
const char *Boot0Tips1[MAX_LANGUAGE] = { \
    ...
    "노란색 화살표로 표시된 베드 고정 나사 a, b, c를 제거해 주세요.",\
};
...
const char *RootPolicyContent[MAX_LANGUAGE] = { \
    "Root免责声明及风险提示\n用户Root设备前，请务必认真阅读该免责声明中的各项条款。通过该免责声明，对用户使用Root功能可能包含的风险及对用户系统的影响进行提示，如果用户选择接受并继续使用Root，则视为用户同意并认可对用户因使用Root功能所带来的风险与损害不承担任何责任。\n风险提示\n获取Root权限后，用户所购买的本产品有可能失去官方保修服务，请用户慎重考虑是否需要获取Root权限获取Root权限后的影响功能\n1.获取Root权限后，将开放本产品系统的最高权限 ，可以操作系统中的所有对象，常规的软件可通过使用Root权限来实现更多功能，但恶意软件在入侵系统时也要使用Root权限。所以请用户慎重考虑授权未知应用使用Root权限；\n2.获取Root权限后，用户可以对本产品中的系统文件进行修改、删除、移动、复制等操作，这些操作可能会引起本产品系统或其他应用程序运行异常；\n3.获取Root权限后，用户可以对预置的系统程序 进行删除，预置的系统程序除了可视化的应用程序外，还包括一些重要的系统组件，删除这些重要系统程序可能会导致系统不稳定和程序运行异常;\n4.获取Root权限后，如果用户授权一些未知程序使用Root权限，可能会造成本产品中的用户信息泄漏，存在隐私信息安全风险；\n免责范围\n用户在使用获取Root权限时，对获取Root权限操作过程及获取Root权限后带来的风险由用户自行承担。在适用法律允许的最大范围内，对因使用或不能使用本产品所产生的损害及风险，我方不承担任何责任；对于产品使用过程中，出现的问题或者异常，包括但不限于产品系统故障、无法执行指令、文件丢失、我方不承诺提供相关技术支持服务。\n\n\n",\
    ...
    "Root-Haftungsausschluss und Risikowarnung\nBitte lesen Sie vor dem Rooten Ihres Geräts die Bedingungen dieses Haftungsausschlusses sorgfältig durch. Dieser Haftungsausschluss erinnert den Benutzer an die potenziellen Risiken bei Verwendung der Root-Funktion und deren Auswirkungen auf das System des Benutzers. Wenn Sie sich dafür entscheiden, Root zu akzeptieren und fortzusetzen, sollte dies als Benutzervereinbarung behandelt und alle unten aufgeführten Bedingungen bestätigt werden. Benutzer tragen alle potenziellen Risiken und die Verantwortung für versehentliche Schäden durch die Verwendung dieser Root-Funktion für sich selbst.\nRisikowarnung\nDas Rooten kann Ihre GARANTIE FÜR DIESES PRODUKT ungültig machen, und Sie sollten sorgfältig überlegen, ob Sie Root-Berechtigungen benötigen.\nRisiken des Rootens\nNach dem Erhalt des Root-Zugriffs wird der höchste Zugriff auf das System dieses PRODUKTS geöffnet, der auf alle Betriebssystemobjekte zugreifen kann. Allgemeine Software kann durch Verwendung des Root-Zugriffs weitere Funktionen erreichen, aber auch Malware muss den Root-Zugriff verwenden, wenn sie in das System eindringt. Bitte überlegen Sie sorgfältig, ob Sie unbekannten Anwendungen die Verwendung der Root-Berechtigung erlauben möchten.\nNach dem Erhalt des Root-Zugriffs können Benutzer Systemdateien in diesem PRODUKT ändern, löschen, verschieben, kopieren usw. Alle diese Operationen können dazu führen, dass das System dieses PRODUKTS oder andere Anwendungen Ausnahmen verursachen.\nNach dem Erhalt des Root-Zugriffs können Benutzer das vorinstallierte Programm neben den Visualisierungsanwendungen löschen und auch mehrere wichtige Systemkomponenten umfassen. Das Entfernen dieser wichtigen Systemprogramme kann zu Systeminstabilität und abnormalen Programmverhalten führen.\nNach dem Erhalt des Root-Zugriffs können die Benutzer, wenn sie einigen unbekannten Programmen die Verwendung der Root-Berechtigung gestatten, die Benutzerdaten in diesem PRODUKT preisgeben und ein Sicherheitsrisiko für private Informationen verursachen.\nHaftungsausschlussumfang\nBei Verwendung der Root-Berechtigung tragen Benutzer die Verantwortung für alle Root-Risiken während des Privilegierungsprozesses und nach Erhalt des Root-Privilegs. Soweit gesetzlich zulässig, haften wir nicht für Schäden und Risiken, die sich aus der Verwendung oder Unfähigkeit zur Verwendung dieses Produkts ergeben. Für Probleme oder Abweichungen, die während der Verwendung des Produkts auftreten, einschließlich, aber nicht beschränkt auf Ausfall des Produktsystems, Nichtausführbarkeit von Anweisungen, Dateiverlust, versprechen wir nicht, entsprechende technische Unterstützungsdienste bereitzustellen.\n\n\n",\
    "Aviso de exención de responsabilidad y advertencia de riesgos de root\nAntes de rootear su dispositivo, lea los términos de esta exención de responsabilidad detenidamente. Esta exención de responsabilidad recuerda al usuario los posibles riesgos involucrados en el uso de la función de root y el impacto en el sistema del usuario. Si decide aceptar y continuar usando Root, se considerará como el acuerdo del usuario y confirmará todos los términos a continuación. Los usuarios asumirán todos los posibles riesgos y la responsabilidad por los daños accidentales al utilizar esta función de root para sí mismos.\nAdvertencia de riesgo\nRootear puede anular la garantía de ESTE PRODUCTO, y debe considerar cuidadosamente si necesita obtener permisos de root.\nRiesgos de root\nDespués de obtener acceso root, se abrirá el privilegio más alto del sistema de ESTE PRODUCTO, lo que permite acceder a todos los objetos del sistema operativo. El software general puede lograr más funciones utilizando el acceso root, pero el malware también debe utilizar el acceso root al invadir el sistema. Por lo tanto, considere cuidadosamente otorgar permisos de root a aplicaciones desconocidas.\nDespués de obtener acceso root, los usuarios pueden modificar, eliminar, mover, copiar, etc. los archivos del sistema de ESTE PRODUCTO. Todas estas operaciones pueden causar excepciones en el sistema de ESTE PRODUCTO u otras aplicaciones.\nDespués de obtener acceso root, los usuarios pueden eliminar programas preinstalados además de las aplicaciones de visualización, lo que incluye también varios componentes importantes del sistema. Eliminar estos programas importantes del sistema puede causar inestabilidad del sistema y un comportamiento anormal del programa.\nDespués de obtener acceso root, si los usuarios otorgan permisos de root a algunos programas desconocidos, la información de privacidad del usuario en ESTE PRODUCTO puede filtrarse y resultar en un riesgo de seguridad para la información privada.\nÁmbito de la exención de responsabilidad\nAl utilizar el permiso de root, los usuarios deben asumir la responsabilidad de todos los riesgos de root durante el proceso de operación de privilegios y después de obtener el privilegio de root. En la medida máxima permitida por la ley aplicable, no seremos responsables de ningún daño y riesgo derivados del uso o la incapacidad de utilizar este producto; Para problemas o anormalidades que ocurran durante el uso del producto, incluidos, entre otros, fallos del sistema del producto, incapacidad para ejecutar instrucciones y pérdida de archivos, no prometemos proporcionar servicios de soporte técnico relevantes.\n\n\n",\
    "Clause de non-responsabilité et avertissement des risques de root\nAvant de rooter votre appareil, veuillez lire attentivement les termes de cette clause de non-responsabilité. Cette clause de non-responsabilité rappelle à l'utilisateur les risques potentiels liés à l'utilisation de la fonction Root et les impacts sur le système de l'utilisateur. Si vous choisissez d'accepter et de continuer à utiliser Root, cela sera considéré comme un accord de l'utilisateur et vous confirmez tous ces termes ci-dessous, les utilisateurs assumeront tous les risques potentiels et la responsabilité des dommages accidentels en utilisant cette fonction Root pour eux-mêmes.\nAvertissement des risques\nLe rootage peut annuler la garantie de CE PRODUIT, et vous devez considérer attentivement si vous avez besoin d'obtenir une autorisation de root.\nRisques de rootage\nAprès avoir obtenu un accès root, cela ouvrira le plus haut niveau de privilège du système CE PRODUIT, ce qui peut accéder à tous les objets du système d'exploitation. Les logiciels généraux peuvent réaliser plus de fonctions en utilisant l'accès root, mais les logiciels malveillants peuvent également utiliser l'accès root lorsqu'ils envahissent le système. Veuillez donc considérer attentivement l'autorisation d'applications inconnues à utiliser l'autorisation root.\nAprès avoir obtenu un accès root, les utilisateurs peuvent modifier, supprimer, déplacer, copier, etc. les fichiers système dans CE PRODUIT. Toutes ces opérations peuvent entraîner des exceptions du système CE PRODUIT ou d'autres applications.\nAprès avoir obtenu un accès root, les utilisateurs peuvent supprimer les programmes préinstallés en plus des applications de visualisation, et cela inclut également plusieurs composants importants du système. La suppression de ces programmes importants du système peut causer une instabilité du système et un comportement anormal du programme.\nAprès avoir obtenu un accès root, si les utilisateurs autorisent certains programmes inconnus à utiliser l'autorisation root, les informations de confidentialité de l'utilisateur dans CE PRODUIT peuvent être divulguées et entraîner un risque de sécurité des informations privées.\nPortée de la clause de non-responsabilité\nLors de l'utilisation de l'autorisation de root, les utilisateurs doivent assumer la responsabilité de tous les risques de root lors du processus d'exploitation des privilèges et après avoir obtenu le privilège de root. Dans toute la mesure permise par la loi applicable, nous ne serons pas responsables des dommages et risques découlant de l'utilisation ou de l'incapacité d'utiliser ce produit ; Pour les problèmes ou anomalies survenant lors de l'utilisation du produit, y compris, sans s'y limiter, les défaillances du système du produit, l'impossibilité d'exécuter des instructions et la perte de fichiers, nous ne promettons pas de fournir des services de support technique pertinents.\n\n\n",\
    "Avviso e avvertenza di responsabilità radice\nPrima di eseguire il root del tuo dispositivo, leggi attentamente i termini di questo avviso. Questo avviso ricorda all'utente dei potenziali rischi derivanti dall'utilizzo della funzione Root e dell'impatto sul sistema dell'utente. Se scegli di accettare e continuare a utilizzare Root, dovrai considerarlo come un accordo dell'utente e confermare tutti i seguenti termini. Gli utenti si assumeranno tutti i potenziali rischi e la responsabilità per eventuali danni accidentali derivanti dall'utilizzo di questa funzione Root per conto proprio.\nAvvertenza sui rischi\nIl root potrebbe invalidare la garanzia di QUESTO PRODOTTO e dovresti valutare attentamente se hai bisogno di ottenere i permessi di root.\nRischi del root\nDopo aver ottenuto l'accesso root, si otterranno i privilegi più elevati del sistema di QUESTO PRODOTTO, che possono accedere a tutti gli oggetti del sistema operativo. Il software generale può svolgere più funzioni utilizzando l'accesso root, ma anche il malware può utilizzare l'accesso root per invadere il sistema. Pertanto, valuta attentamente l'autorizzazione ad applicazioni sconosciute per l'utilizzo dei permessi di root.\nDopo aver ottenuto l'accesso root, gli utenti possono modificare, eliminare, spostare, copiare, ecc. i file di sistema di QUESTO PRODOTTO. Tutte queste operazioni potrebbero causare il malfunzionamento del sistema di QUESTO PRODOTTO o delle altre applicazioni.\nDopo aver ottenuto l'accesso root, gli utenti possono eliminare programmi preinstallati oltre alle applicazioni di visualizzazione, inclusi anche numerosi componenti di sistema importanti. La rimozione di questi programmi di sistema importanti potrebbe causare l'instabilità del sistema e il comportamento anomalo del programma.\nDopo aver ottenuto l'accesso root, se gli utenti autorizzano alcuni programmi sconosciuti a utilizzare i permessi di root, le informazioni sulla privacy dell'utente in QUESTO PRODOTTO potrebbero essere divulgate e comportare un rischio per la sicurezza delle informazioni private.\nAmbito di esclusione di responsabilità\nDurante l'utilizzo dei permessi di root, gli utenti devono assumersi la responsabilità di tutti i rischi del root durante il processo di operazione dei privilegi e dopo aver ottenuto il privilegio di root. Nella misura massima consentita dalla legge applicabile, non saremo responsabili per eventuali danni e rischi derivanti dall'utilizzo o dall'incapacità di utilizzare questo prodotto; Per i problemi o le anomalie che si verificano durante l'utilizzo del prodotto, compresi, ma non limitati a, il malfunzionamento del sistema del prodotto, l'incapacità di eseguire istruzioni, la perdita di file, non promettiamo di fornire servizi di supporto tecnico pertinenti.\n\n\n",\
    "Aviso de Isenção de Responsabilidade e Risco da Raiz\nAntes de fazer root no seu dispositivo, leia atentamente os termos desta declaração de isenção de responsabilidade. Esta declaração lembra o usuário dos riscos potenciais envolvidos no uso da função de root e do impacto no sistema do usuário. Se você optar por aceitar e continuar usando o root, isso deve ser tratado como o acordo do usuário e confirmar todos os termos abaixo, os usuários assumirão todos os riscos potenciais e a responsabilidade por danos acidentais pelo uso desta função de root para si próprios.\nAviso de Risco\nFazer root pode anular a garantia deste PRODUTO e você deve considerar cuidadosamente se precisa obter permissão de root.\nRiscos do Rooting\nApós obter acesso ao root, ele abrirá o privilégio mais alto do sistema deste PRODUTO, que pode acessar todos os objetos do sistema operacional. O software geral pode obter mais funções usando o acesso ao root, mas o malware também precisa usar o acesso ao root ao invadir o sistema. Portanto, considere cuidadosamente autorizar aplicativos desconhecidos a usar a permissão de root.\nApós obter acesso ao root, os usuários podem modificar, excluir, mover, copiar, etc. os arquivos do sistema neste PRODUTO. Todas essas operações podem causar exceções no sistema deste PRODUTO ou em outros aplicativos.\nApós obter acesso ao root, os usuários podem excluir o programa pré-instalado, além de aplicativos de visualização, e inclui também vários componentes importantes do sistema. Remover esses programas importantes do sistema pode causar instabilidade no sistema e comportamento anormal do programa.\nApós obter acesso ao root, se os usuários autorizarem alguns programas desconhecidos a usar a permissão de root, as informações de privacidade do usuário neste PRODUTO podem vazar e resultar em um risco de segurança das informações privadas.\nEscopo da Isenção de Responsabilidade\nAo usar a permissão de root, os usuários devem assumir a responsabilidade por todos os riscos de root durante o processo de operação de privilégio e após obter o privilégio de root. Na medida máxima permitida pela lei aplicável, não seremos responsáveis por quaisquer danos e riscos decorrentes do uso ou incapacidade de usar este produto; Para problemas ou anormalidades que ocorram durante o uso do produto, incluindo, mas não se limitando a falha do sistema do produto, incapacidade de executar instruções, perda de arquivos, não prometemos fornecer serviços de suporte técnico relevantes.\n\n\n",\
    "Корневой отказ от ответственности и предупреждение о рисках\nПеред получением root-доступа к вашему устройству внимательно ознакомьтесь с условиями этого отказа от ответственности. Этот отказ от ответственности напоминает пользователю о потенциальных рисках, связанных с использованием функции Root, и их влиянии на систему пользователя. Если вы решите принять и продолжить использование Root, это должно рассматриваться как пользовательское соглашение и подтверждать все нижеперечисленные условия, пользователи несут все потенциальные риски и ответственность за случайные повреждения при использовании этой функции Root.\nПредупреждение о рисках\nПолучение root-доступа может аннулировать гарантию на ЭТОТ ПРОДУКТ, и вам следует тщательно обдумать, нужно ли вам получать root-разрешение.\nРиски при получении Root\nПосле получения root-доступа он открывает самые высокие привилегии системы ЭТОГО ПРОДУКТА, которые могут получить доступ ко всем объектам операционной системы. Обычное программное обеспечение может выполнять больше функций, используя root-доступ, но вредоносное ПО также должно использовать root-доступ при вторжении в систему. Поэтому тщательно обдумайте разрешение на использование root для неизвестных приложений.\nПосле получения root-доступа пользователи могут изменять, удалять, перемещать, копировать и т. д. системные файлы в ЭТОМ ПРОДУКТЕ. Все эти операции могут вызвать сбои в системе ЭТОГО ПРОДУКТА или других приложениях.\nПосле получения root-доступа пользователи могут удалить предустановленные программы, а также визуализационные приложения, а также включить ряд важных системных компонентов. Удаление этих важных системных программ может вызвать нестабильность системы и ненорм\n\n\n",\
    "Root Feragatname ve Risk Uyarısı\nCihazınızı rootlamadan önce bu feragatnamenin şartlarını dikkatlice okuyun. Bu feragatname, kullanıcıya Root işlevinin kullanımında ortaya çıkabilecek potansiyel riskler ve kullanıcının sistemine etkileri konusunda hatırlatmada bulunur. Root'u kabul edip kullanmaya devam etmeyi seçerseniz, bu kullanıcı sözleşmesi olarak kabul edilmeli ve aşağıdaki tüm şartları onaylamalısınız; kullanıcılar bu Root işlevini kullanarak tüm potansiyel riskleri ve kazara oluşabilecek hasarları kendi sorumlulukları altına alacaklardır.\nRisk Uyarısı\nRootlama işlemi CE ÜRÜN garantinizi geçersiz kılabilir ve root erişimi elde etmeniz gerekip gerekmediğini iyice düşünmelisiniz.\nRootlama Riskleri\nRoot erişimi elde ettikten sonra, CE ÜRÜN sisteminin en yüksek ayrıcalığını açar ve tüm işletim sistemi nesnelerine erişebilir. Genel yazılımlar Root erişimini kullanarak daha fazla işlev gerçekleştirebilir, ancak kötü amaçlı yazılımlar da sistemlere saldırdığında Root erişimini kullanmak zorundadır. Bu nedenle, bilinmeyen uygulamaların Root izni kullanmasını dikkatlice düşünün.\nRoot erişimi elde ettikten sonra, kullanıcılar CE ÜRÜN'deki sistem dosyalarını değiştirebilir, silebilir, taşıyabilir, kopyalayabilir vb. Tüm bu işlemler CE ÜRÜN sisteminde veya diğer uygulamalarda istisnalara neden olabilir.\nRoot erişimi elde ettikten sonra, kullanıcılar görselleştirme uygulamaları dışında önceden yüklenmiş programları ve birçok önemli sistem bileşenini silebilir. Bu önemli sistem programlarının kaldırılması sistem kararsızlığına ve anormal program davranışına neden olabilir.\nRoot erişimi elde ettikten sonra, kullanıcılar bilinmeyen bazı programlara Root izni verirse, CE ÜRÜN'deki kullanıcı gizlilik bilgileri sızabilir ve özel bilgilerin güvenlik riskine neden olabilir.\nFeragatname Kapsamı\nRoot izni kullanıldığında, kullanıcılar ayrıcalık işlemi sürecinde ve root ayrıcalığı elde ettikten sonra tüm root risklerinden sorumlu olmalıdır. Uygulan\n\n\n",\
    "ルート免責事項とリスク警告\nデバイスをルート化する前に、この免責事項の条件をよくお読みください。この免責事項は、ルート機能の使用に伴う潜在的なリスクとユーザーのシステムへの影響について、ユーザーに注意を喚起します。Rootを選択し続ける場合は、ユーザー契約として取り扱われ、以下のすべての条件を確認し、利用者はこのRoot機能を使用することによるすべての潜在的なリスクと偶発的な損害の責任を負うものとします。\nリスク警告\nルート化は、この製品の保証を無効にする可能性があり、ルート権限を取得する必要があるかどうかを慎重に考える必要があります。\nルート化のリスク\nルートアクセスを取得した後、これによりこの製品のシステムの最高権限が開かれ、すべてのオペレーティングシステムオブジェクトにアクセスすることができます。一般的なソフトウェアは、ルートアクセスを使用することでより多くの機能を実現できますが、マルウェアもシステムに侵入する際にルートアクセスを使用する必要があります。したがって、不明なアプリケーションにルート権限の使用を許可するかどうかを慎重に考えてください。\nルートアクセスを取得した後、ユーザーはこの製品内のシステムファイルを変更、削除、移動、コピーなどすることができます。これらの操作は、この製品のシステムまたは他のアプリケーションの例外を引き起こす可能性があります。\nルートアクセスを取得した後、ユーザーは視覚化アプリケーションに加えて、プリインストールされたプログラムや重要なシステムコンポーネントを削除することもできます。これらの重要なシステムプログラムを削除すると、システムの不安定化やプログラムの異常な動作が発生する可能性があります。\nルートアクセスを取得した後、ユーザーがいくつかの不明なプログラムにルート権限の使用を許可すると、この製品内のユーザーのプライバシー情報が漏洩し、プライベート情報のセキュリティリスクが発生する可能性があります。\n免責事項の範囲\nルート権限を使用する際には、特権操作プロセス中およびルート権限を取得した後のすべてのルートリスクについて責任を負うものとします。適用法において許容される限度において、当社はこの製品の使用または使用できないことに起因するいかなる損害やリスクについても責任を負いません。製品の使用中に発生する問題や異常、製品のシステム障害、命令の実行不能、ファイルの紛失などについて、関連する技術サポートサービスを提供することを約束しません。\n\n\n"
    "루트 비고 및 위험 경고\n기기를 루트화하기 전에 이 면책 조항의 조건을 주의 깊게 읽으십시오. 이 면책 조항은 루트 기능 사용 시 발생 가능한 위험과 사용자 시스템에 미칠 영향에 대해 사용자에게 알립니다. 루트를 선택하고 계속 사용하기로 결정한 경우, 사용자 동의서로 간주되며 아래의 모든 조건을 확인하고 모든 잠재적인 위험과 우발적인 손상에 대한 책임을 사용자가 져야 합니다.\n위험 경고\n루트화는 이 제품의 보증을 무효화할 수 있으며, 루트 권한을 얻을 필요성을 신중히 고려해야 합니다.\n루트화의 위험\n루트 액세스를 얻은 후, 시스템의 최고 권한을 열어 모든 운영 체제 개체에 액세스할 수 있습니다. 루트 액세스를 사용하면 일반 소프트웨어는 더 많은 기능을 달성할 수 있지만, 악성 소프트웨어도 시스템 침입시 루트 액세스를 사용해야 합니다. 따라서 알 수 없는 애플리케이션에게 루트 권한 사용을 승인할 때 신중히 고려하십시오.\n루트 액세스를 얻은 후, 사용자는 이 제품의 시스템 파일을 수정, 삭제, 이동, 복사 등 할 수 있습니다. 이러한 모든 작업은 이 제품 시스템이나 다른 애플리케이션에서 예외를 발생시킬 수 있습니다.\n루트 액세스를 얻은 후, 사용자는 시각화 애플리케이션 외에도 설치된 프로그램을 삭제할 수 있으며, 일부 중요한 시스템 구성 요소도 포함됩니다. 이러한 중요한 시스템 프로그램을 제거하는 경우 시스템의 불안정성과 프로그램의 비정상적인 동작을 초래할 수 있습니다.\n루트 액세스를 얻은 후, 사용자가 알 수 없는 일부 프로그램에게 루트 권한 사용을 승인하는 경우, 이 제품의 사용자 개인 정보가 유출되어 개인 정보의 보안 위험이 발생할 수 있습니다.\n면책 조항 범위\n루트 권한을 사용하는 동안 사용자는 권한 작업 과정 및 루트 권한 획득 후의 모든 루팅 위험에 대한 책임을 져야 합니다. 관련 법률이 허용하는 최대한도 내에서, 이 제품의 사용 또는 사용 불능으로 인해 발생하는 모든 손해와 위험에 대해 책임지지 않습니다. 제품 사용 중 발생하는 문제나 비정상적인 현상, 제품 시스템 장애, 명령 실행 불능, 파일 손실 등에 대한 관련 기술 지원 서비스를 제공하는 것을 약속하지 않습니다.",\
};
...
const char *WikiTips[MAX_LANGUAGE] = { \
    ...
    "휴대폰으로 아래 QR 코드를 스캔하여 온라인 메뉴얼을 확인하세요.",\
};
...
const char *FaqTips[MAX_LANGUAGE] = { \
    ...
    "휴대폰으로 아래 QR 코드를 스캔하여 FAQ를 확인하세요.",\
};
/*********************boot*****************************/
...
const char * Account[MAX_LANGUAGE] = { \
    "账号",\
    "Account",\
    "Konto",\
    "Cuenta",\
...
const char * PassWord[MAX_LANGUAGE] = { \
    "密码",\
    "Password",\
    "Passwort",\
    "Contraseña",\
...
const char *FilamentTips[MAX_LANGUAGE] = { \
    "提示",\
    "Tips",\
    "Tipps",\
    "Consejos",\
...
const char *LastPageMsg[MAX_LANGUAGE] = { \
    "已经是尾页", \
    "Already on trailer page", \
    "Bereits auf der Rückseite", \
    "Ya se encuentra en la página tráiler", \
    "Déjà sur la page de garde", \
    "Già nella pagina del trailer", \
    "Já está na página de trás", \
    "Уже на последней странице", \
    "Zaten artbilgi sayfasında", \
    "既に最終ページです",\
    "마지막 페이지",\
};
...
const char *FirstPageMsg[MAX_LANGUAGE] = { \
    ...
    "Ya se encuentra en página frontal", \
    "Déjà sur la marche", \
    "Già in prima pagina", \
    "Já está na página da frente", \
...
const char *Screening[MAX_LANGUAGE] = { \
    "筛查", \
    "Screening", \
    "Screening", \
    "Cribado", \
...
const char *SortBySize[MAX_LANGUAGE] = { \
    "按大小", \
    "By Size", \
    "Nach Größe", \
    "Por Tamaño", \
...
const char *SortByTime[MAX_LANGUAGE] = { \
    "按时间", \
    "By Time", \
    "Nach Zeit", \
    "Por Tiempo", \
...
const char *SortByName[MAX_LANGUAGE] = { \
    "按名称（A~Z)", \
    "By Name (A~Z)", \
    "Nach Name (A~Z)", \
    "Por Nombre (A~Z)", \
...
const char *PrintingTips[MAX_LANGUAGE] = { \
    ...
    "印刷中につき、当分操作不可",\
    "출력 중에는 허용되지 않은 작업입니다.",\
};

const char *PrintingTips_Pauseprint[MAX_LANGUAGE] = { \
    "暂停打印中，请勿操作...", \
    "Printing is paused, please do not operate...", \
    "Printing is paused, please do not operate...", \
    "Printing is paused, please do not operate...", \
    "Printing is paused, please do not operate...", \
    "Printing is paused, please do not operate...", \
...
const char *PrintingTips_Resumeprint[MAX_LANGUAGE] = { \
    "恢复打印中，请勿操作...", \
    "Resuming printing, please do not operate...", \
    "Resuming printing, please do not operate...", \
    "Resuming printing, please do not operate...", \
    "Resuming printing, please do not operate...", \
    "Resuming printing, please do not operate...", \
...
const char *PrintingTips_Stopprint[MAX_LANGUAGE] = { \
    "停止打印中，请勿操作...", \
    "Stop printing, please do not operate...", \
    "Stop printing, please do not operate...", \
    "Stop printing, please do not operate...", \
    "Stop printing, please do not operate...", \
    "Stop printing, please do not operate...", \
...
const char *PrintingTips_Startprint[MAX_LANGUAGE] = { \
    "开始打印中，请勿操作...", \
    "Starting to print, please do not operate...", \
    "Starting to print, please do not operate...", \
    "Starting to print, please do not operate...", \
    "Starting to print, please do not operate...", \
    "Starting to print, please do not operate...", \
...
const char *PrintingTips_CantPause[MAX_LANGUAGE] = { \
    "打印校准中，暂不允许暂停", \
    "Printing calibration, temporarily not allowed to pause", \
    "Printing calibration, temporarily not allowed to pause", \
    "Printing calibration, temporarily not allowed to pause", \
    "Printing calibration, temporarily not allowed to pause", \
    "Printing calibration, temporarily not allowed to pause", \
...
const char *AdvancedMode[MAX_LANGUAGE] = { \
    "专家模式",\
    "Expert mode",\
    "Expertenmodus",\
    "Modo experto",\
...
const char *Zoffset[MAX_LANGUAGE] = { \
    "Z offset",\
    "Z offset",\
    "Z offset",\
    "Z offset",\
...
const char *BedMeshAdjust[MAX_LANGUAGE] = { \
    "床网调整",\
    "Bed mesh adjustment",\
    "Bettgestellanpassung",\
    "Ajuste de cama con red",\
    "Ajustement du lit avec filet",\
    "Regolazione della rete del letto",\
    "Ajuste de rede de cama",\
    "Регулировка кроватной сетки",\
    "Yatak ağı ayarı",\
    "ベッドネットの調整",\
    "베드넷 조정",\
};

const char *NozzlePidDetect[MAX_LANGUAGE] = { \
    "PID喷头检测",\
    "Nozzle PID calibration",\
    "Nozzle PID-Kalibrierung",\
    "Calibración del PID de la ZONA nozzle",\
    "Étalonnage du PID nozzle",\
    "Calibrazione PID del getto",\
    "Calibración del PID de la ZONA nozzle",\
    "Калибровка PID насадки",\
    "Nozül PID kalibrasyonu",\
    "ホットエンドPIDキャリブレーション",\
    "노즐 PID 캘리브레이션",\
};

const char *PIDNozzleCheckPrepareTip[MAX_LANGUAGE] = { \
    "PID喷头检测预计需要5分钟左右，检测过程中请勿断电！",\
    "Nozzle PID calibration is expected to take around 5 minutes, please do not power down during the test!",\
    "Nozzle PID-Kalibrierung wird voraussichtlich ca. 5 Minuten dauern, bitte schalten Sie während der Prüfung nicht ab!",\
    "La calibración del PID nozzle tardará unos 5 minutos, ¡no lo apague durante la prueba!",\
    "L'étalonnage du PID nozzle devrait prendre environ 5 minutes, ne pas éteindre l'appareil pendant le test !",\
    "La calibrazione del PID nozzle dovrebbe richiedere circa 5 minuti, non spegnere durante il test!",\
    "A verificação do bico PID deverá levar cerca de 5 minutos. Por favor, não desligue o aparelho durante o processo de teste!",\
    "Тест сопла PID ожидается займет около 5 минут. Во время процесса тестирования, пожалуйста, не выключайте питание!",\
    "PID nozül testinin yaklaşık 5 dakika sürmesi bekleniyor. Test süreci boyunca lütfen gücü kesmeyin!",\
    "ホットエンドのPIDキャリブレーションは約５分かかる見込みです。テスト中は電源を落とさないようにお願いします",\
    "노즐 PID 캘리브레이션은 약 5분 소요되며 이 과정에서 전원을 끄지 마세요.",\
};

const char *PIDNozzleCheckingTip[MAX_LANGUAGE] = { \
    "正在进行喷头PID检查及校准",\
    "Nozzle PID check and calibration in progress",\
    "PID-Prüfung und Kalibrierung des nozzle ist im Gange",\
    "Comprobación y calibración del PID del nozzle en curso.",\
    "Contrôle et calibrage du PID de la buse en cours",\
    "Controllo e calibrazione del PID dell'ugello in corso",\
    "Nozzle PID check and calibration in progress",\
    "Проверка и калибровка сопла PID в процессе",\
    "Nozul PID kontrol ve kalibrasyonu devam ediyor",\
    "ノズルPIDのチェックとキャリブレーションが進行中",\
    "노즐 PID 검사 및 캘리브레이션 진행 중",\
};

const char *PIDNozzleCheckingTimeTip[MAX_LANGUAGE] = { \
    "预计需要5分钟左右。",\
    "It is expected to take a total of 5 minutes",\
    "Es wird erwartet, dass der Selbsttest insgesamt 5 Minuten dauert.",\
    "Está previsto que tarde un total de 5 minutos",\
    "L'autotest devrait durer 5 minutes au total.",\
    "Si stima che ci vorranno un totale di 5 minuti",\
    "Espera-se que leve um total de 5 minutos",\
    "Ожидается, что это займет всего 5 минут.",\
    "Toplamda 5 dakika alması bekleniyor.",\
    "計5分かかる見込みです。",\
    "약 5분 소요됩니다.",\
};

const char *PIDNozzleCheckedTip[MAX_LANGUAGE] = { \
    "喷头PID检查及校准已完成",\
    "喷头PID检查及校准已完成",\
    "喷头PID检查及校准已完成",\
    "喷头PID检查及校准已完成",\
...
const char *PIDNozzleCheckedTip1[MAX_LANGUAGE] = { \
    "暂未发现异常。",\
    "No abnormalities found yet",\
    "Noch keine Abnormalität gefunden",\
    "Ninguna anomalía encontrada todavía",\
    "Aucune anomalie n'a encore été détectée",\
    "Ancora nessuna anomalia trovata",\
    "Nenhuma anormalidade encontrada ainda",\
    "Аномалий пока не обнаружено.",\
    "Henüz bir anormallik bulunamadı.",\
    "まだ異常は見つかっていません",\
    "이상 없음",\
};

const char *BedPidDetect[MAX_LANGUAGE] = { \
    "PID热床检测",\
    "HOTBED PID calibration",\
    "HEIZBETT PID-Kalibrierung",\
    "Calibración del PID de la ZONA CALIENTE",\
    "Étalonnage du PID du HOTBED",\
    "Calibrazione PID del LETTO CALDO",\
    "Calibragem do PID da MESA",\
    "Проверка тепловой кровати PID",\
    "PID yatak tespiti",\
    "ヒートベッドPIDキャリブレーション",\
    "핫베드 PID 캘리브레이션",\
};

const char *PIDBedCheckPrepareTip[MAX_LANGUAGE] = { \
    "PID热床检测预计需要10分钟左右，检测过程中请勿断电！",\
    "HOTBED PID calibration is expected to take about 10 minutes, do not power down during the inspection!",\
    "HOTBED PID-Kalibrierung wird voraussichtlich etwa 10 Minuten dauern, schalten Sie das Gerät während der Prüfung nicht aus!",\
    "La calibración del PID HOTBED tardará unos 10 minutos, ¡no lo apague durante la inspección!",\
    "L'étalonnage du PID HOTBED devrait prendre environ 10 minutes, ne pas éteindre l'appareil pendant l'inspection!",\
    "Ci si aspetta che il rilevamento del letto PID richieda circa 10 minuti. Si prega di non spegnere durante il processo di rilevamento!",\
    "Espera-se que a calibração HOTBED PID leve cerca de 10 minutos, não desligue durante a inspeção!",\
    "Ожидается, что проверка PID-принтера займет около 10 минут. Пожалуйста, не выключайте питание во время проверки!",\
    "PID yatak tespitinin yaklaşık 10 dakika sürmesi bekleniyor. Lütfen tespit sürecinde elektriği kesmeyin!",\
    "ヒートベッドのPIDキャリブレーションは約10分かかる見込みです。テスト中は電源を落とさないようにお願いします",\
    "핫베드 PID 캘리브레이션은 약 5분 소요되며 이 과정에서 전원을 끄지 마세요.",\
};

const char *PIDBedCheckingTip[MAX_LANGUAGE] = { \
    "正在进行热床PID检查及校准",\
    "Hot bed PID check and calibration in progress",\
    "PID-Prüfung und Kalibrierung des Hotbeds ist im Gange",\
    "Comprobación y calibración del PID de la cama caliente en curso",\
    "Vérification du PID du plateau chauffant et étalonnage en cours",\
    "In corso il controllo e la calibrazione PID del piano riscaldante",\
    "Verificação e calibração do PID de superfície quente em curso",\
    "Выполняется проверка и калибровка PID греющей платформы",\
    "Sıcak yatak PID kontrolü ve kalibrasyonu yapılıyor",\
    "ヒートベッドのPID調整中",\
    "핫베드 PID 검사 및 캘리브레이션 진행 중",\
};

const char *PIDBedCheckingTimeTip[MAX_LANGUAGE] = { \
    "预计需要15分钟左右。",\
    "It is expected to take a total of 15 minutes",\
    "Es wird erwartet, dass der Selbsttest insgesamt 15 Minuten dauert.",\
    "Está previsto que tarde un total de 15 minutos",\
    "L'autotest devrait durer 15 minutes au total.",\
    "Si stima che ci vorranno un totale di 15 minuti",\
    "Espera-se que leve um total de 15 minutos",\
    "Ожидается, что это займет всего 15 минут.",\
    "Toplamda 15 dakika alması bekleniyor.",\
    "計15分かかる見込みです。",\
    "약 15분 소요됩니다.",\
};

const char *PIDBedCheckedTip[MAX_LANGUAGE] = { \
    "热床PID检查及校准已完成",\
    "Hot bed PID check and calibration completed",\
    "PID-Prüfung und -Kalibrierung des Hotbeds abgeschlossen",\
    "Comprobación y calibración del PID de la cama caliente completadas",\
    "Vérification et étalonnage du PID du plateau chauffant terminés",\
    "Comprobación y calibración del PID de la cama caliente completadas",\
    "Verificação e calibração do PID de superfície quente concluídas",\
    "Проверка и калибровка PID горячей кровати завершены.",\
    "Sıcak yatak PID kontrolü ve kalibrasyonu tamamlandı.",\
    "ヒートベッドのPID調整完了",\
    "핫베드 PID 검사 및 캘리브레이션 완료",\
};

const char *PIDBedCheckedTip1[MAX_LANGUAGE] = { \
    "暂未发现异常。",\
    "No abnormalities found yet",\
    "Noch keine Abnormalität gefunden",\
    "Ninguna anomalía encontrada todavía",\
...
const char *LevelingMethod[MAX_LANGUAGE] = { \
    "调平方式",\
    "Leveling method",\
    "Leveling-Methode",\
...
const char *Automatic[MAX_LANGUAGE] = { \
    "自动",\
    "Automatic",\
    "Automatisch",\
    "Automático",\
...
const char *Manual[MAX_LANGUAGE] = { \
    "手动",\
    "Manual",\
    "Handbuch",\
    "Manual",\
...
const char *AutolevelTips[MAX_LANGUAGE] = { \
    "请点击要调整的区域",\
    "Please click on the area you want to adjust",\
    "请点击要调整的区域",\
    "请点击要调整的区域",\
    "请点击要调整的区域",\
    "请点击要调整的区域",\
...
const char *ResetMachineTips[MAX_LANGUAGE] = { \
    "恢复出厂中，请勿断电...",\
    "Do not power down during factory restoration…",\
    "Schalten Sie die Leistung während der Wiederherstellung auf die Werkseinstellungen nicht ab...",\
    "No retire la alimentación durante la restauración a los valores predeterminados…",\
    "N'éteignez pas l'appareil pendant la restauration d'usine...",\
    "Durante un ripristino delle impostazioni di fabbrica, non perdere potenza...",\
    "Não desligue durante a restauração de fábrica...",\
    "Не отключайте питание во время восстановления заводских настроек…",\
    "Fabrika ayarlarına sıfırlama işlemi sırasında gücü kesmeyin...",\
...
const char *UpdataTips[MAX_LANGUAGE] = { \
    ...
    "펌웨어 버전을 업그레이드 후 다시 레벨링하고 출력을 시작하세요.",\
};
...
const char *Error2[MAX_LANGUAGE] = { \
    "热床加热异常！请按说明书检查。",\
    "Hotbed heating Error! Please check according to the instructions.",\
    "Heizbett Fehler beim Erwärmen! Bitte prüfen Sie anhand der Anweisungen.",\
    "¡Error de calentamiento de sumidero! Compruébelo según las instrucciones.",\
    "Erreur de réchauffement du lit chauffant ! Veuillez vérifier selon les instructions.",\
    "Errore di riscaldamento del piano riscaldante! Errore riscaldamento letto caldo!",\
    "Erro de aquecimento da mesa! Verifique de acordo com as instruções.",\
    "Ошибка нагрева очага! Проверьте, следуя инструкциям.",\
    "Sıcak yatak ısıtma hatası!Sıcak yatak ısıtma hatası!",\
    "ホットベッドの加熱異常を起こしています！取扱説明書に従ってご確認ください。",\
    "베드 가열 오류! 메뉴얼을 확인해 주세요.",\
};  
...
const char *PowerLossRecoverMsg[MAX_LANGUAGE] = { \
    "检测到上次意外停止的文件",\
    "Improper operation detected from last attempt",\
    "Unzulässiger Vorgang beim letzten Versuch erkannt",\
    "Funcionamiento incorrecto detectado desde el último intento",\
    "Opération incorrecte détectée lors de la dernière tentative",\
    "Rilevato file di stampa interrotto inaspettatamente",\
    "Operação inadequada detectada na última tentativa",\
    "При последней попытке обнаружена ненадлежащая работа",\
    "Son denemede yanlış işlem yapıldığı algılandı",\
    "印刷停止のファイルを検出しました",\
    "작업 중지된 파일이 감지되었습니다.",\
};
...
const char *FilamentRunOutMsg[MAX_LANGUAGE] = { \
    "当前耗材用尽，打印已暂停，请更换耗材",\
    "Filament runout, printing paused, please refill.",\
    "Das Filament geht derzeit zur Neige, und der Druck wurde unterbrochen. Bitte tauschen Sie es aus.",\
    "Actualmente se está desgastando el filamento y se ha suspendido la impresión. Sustitúyalo.",\
    "Le filament est actuellement à court et l'impression a été suspendue. Veuillez le remplacer.",\
    "Filamento in esaurimento, stampa sospesa. Sostituirlo.",\
    "Filamente atualmente esgotado, a impressão foi suspensa. Troque-o.",\
    "Нить заканчивается, и печать приостановлена. Замените ее",\
    "Filaman şu anda tükenmek üzere, yazdırma işlemi askıya alındı. Lütfen değiştirin.",\
    "インクがなくなり、印刷が一時停止されましたので、インクを交換してください。",\
    "필라멘트가 부족해 출력이 일시 중지되었습니다. 필라멘트를 교체해 주세요.",\
};
...
const char *ResumePrintTip1[MAX_LANGUAGE] = { \
    "进料中，暂不允许操作", \
    "Extruding, operation not allowed temporarily.", \
    "Extrudieren, vorübergehend keine Operation erlaubt.", \
    "Extrusión, operación no permitida temporalmente.", \
    "Extrusion, opération temporairement non autorisée.", \
    "Estrusione, operazione temporaneamente non consentita.", \
    "Extrusão, operação temporariamente não permitida.", \
    "Экструзия, операция временно не разрешена.", \
    "Ekstrüzyon, geçici olarak işlem yapılamaz.", \
    "押出中、操作は一時的に許可されません。",\
    "필레멘트를 삽입 중 다른 작업을 하지 마세요.",\
};

const char *ResumePrintTip2[MAX_LANGUAGE] = { \
    "退料中，暂不允许操作", \
    "Retracting, operation not allowed at the moment.", \
    "Rücknahme, momentan keine Operation erlaubt.", \
    "Retirando, no se permite la operación en este momento.", \
    "Retrait en cours, opération temporairement interdite.", \
    "Ritiro in corso, operazione non consentita al momento.", \
    "Retirando, operação não permitida no momento.", \
    "Возврат, операция временно не разрешена.", \
    "Geri çekme işlemi devam ediyor, şu anda işlem yapılamaz.", \
    "差し戻し中で、操作は現在許可されていません。",\
    "필레멘트를 배출 중 다른 작업을 하지 마세요.",\
};
...
const char *ModelFan[MAX_LANGUAGE] = { \
    ...
    "Modell Ventilator",\
    "Ventilador modelo",\
    "Ventilateur du Modèle",\
    "Modello ventilatore",\
    "Ventoinha do modelo",\
    "Модельный вентилятор",\
    "Model Fanı",\
    "モデルファン",\
    "메인 팬",\
};
...
const char * HotTips[MAX_LANGUAGE] = { \
    "进退料前温度将自动加热至240℃", \
    "Temperature will heat up to 240°C before feed/retreat", \
    "Temperatur erwärmt sich bis zu 240 °C vor dem Einziehen/Rückziehen", \
    "La temperatura llegará a los 240°C antes de la alimentación / retirada",\
    "La température atteindra 240°C avant l'alimentation/le retraitement.",\
    "La temperatura aumenta automaticamente fino a 240 °C prima dell'alimentazione/ritrazione",\
    "A temperatura será aquecida até 240°C antes da alimentação/retrocesso",\
    "Перед выдавлтв/втянутв температура повысится до 240°C",\
    "Sıcaklık değeri besleme/geri çekme işleminden önce 240°C'ye ulaşacaktır",\
    "フィード/ディスチャージ前の温度は自動的に240℃まで加熱されます",\
    "삽입/배출 전 자동으로 노즐을 240 °C로 가열합니다",\
};
...
const char *RecordIndexTipsMsg[MAX_LANGUAGE] = { \
    "记录详情无法打开", \
    "Why the record details unable to open", \
    "Warum die Datensatzdetails nicht geöffnet werden können", \
    "Por qué no se pueden abrir los detalles de registro", \
    "Raison expliquant la difficulté d'ouvrir les détails de l'enregistrement", \
    "Perché i dettagli della registrazione non possono essere aperti", \
    "Motivo pelo qual os detalhes não podem ser abertos", \
    "Почему не удается открыть подробную информацию записи", \
    "Kayıt detayları neden açılamıyor", \
    "レコード詳細は開けません",\
    "기록 세부사항 열 수 없습니다",\
};
...
const char *LayerStartPos[MAX_LANGUAGE] = { \
    ...
    "Startposition der einzelnen",\
    "Posición de cada nivel",\
    "Position de chaque niveau",\
    "Posizione di ogni livello",\
    "Posição inicial de cada nível",\
    "Начальное каждого уровня",\
    "Her bir başlangıç pozisyonu",\
    "レイヤーごとの開始位置",\
    "다음 레이어 이동 시",\
};
const char *HotendMoveAway[MAX_LANGUAGE] = { \
    "喷头移开时",\
    "When extruder moves away",\
    "Wenn der Extruder wegbewegt wird",\
    "Cuando el extrusor se aleja",\
    "Lorsque extrudeuse est éloignée",\
    "Quando l'estrusore  allontanato",\
    "Quando a extrusora movida",\
...
const char *AIdetection[MAX_LANGUAGE] = { \
    ...
    "#FFFFFF AI 탐지##FFD15B (最適化において)#",\
};

const char *AIShareTips[MAX_LANGUAGE] = { \
    "(分享照片生效中)",\
    "(Photo Sharing is Active)",\
    "(Fotofreigabe ist aktiv)",\
    "(Compartir fotos está activo)",\
    "(Le partage de photos est actif)",\
    "(Condivisione delle foto è attiva)",\
    "(Compartilhamento de fotos está ativo)",\
    "(Поделиться фото активно)",\
    "(Fotoğraf paylaşımı aktif)",\
    "(写真の共有が有効です)",\
    "(사진 공유가 활성화되어 있습니다)",\
};

const char *AIShareCheckboxContent[MAX_LANGUAGE] = { \
    "分享照片帮助提高打印错误检测率#42bdd8 详情#",\
    "Sharing photos helps improve the error detection rate in printing#42bdd8 Details#",\
    "Das Teilen von Fotos hilft, die Fehlersuchrate beim Drucken zu verbessern#42bdd8 Details#",\
    "Compartir fotos ayuda a mejorar la tasa de detección de errores en la impresión#42bdd8 Detalles#",\
    "Le partage de photos aide à améliorer le taux de détection des erreurs d'impression#42bdd8 Détails#",\
    "Condividere foto aiuta a migliorare il tasso di rilevamento degli errori nella stampa#42bdd8 Dettagli#",\
    "Compartilhar fotos ajuda a melhorar a taxa de detecção de erros na impressão#42bdd8 Detalhes#",\
    "Обмен фотографиями помогает улучшить эффективность обнаружения ошибок при печати#42bdd8 Подробности#",\
    "Fotoğrafları paylaşmak, baskı hatalarının tespit oranını artırır#42bdd8 Detaylar#",\
    "写真を共有することで印刷エラーの検出率が向上します#42bdd8 詳細#",\
    "사진을 공유하면 인쇄 오류 검출률이 향상됩니다#42bdd8 세부 사항#",\
};
...
const char *AINormalModeTips[MAX_LANGUAGE] = { \
    "每隔%d s检测一次，可发现较明显的问题",\
    "Detect every %d seconds and identify prominent issues",\
    "Alle %d Sekunden überprüfen und deutliche Probleme erkennen",\
    "Detectar cada %d segundos y encontrar problemas evidentes",\
    "Détecter toutes les %d secondes et identifier les problèmes évidents",\
    "Rilevare ogni %d secondi e individuare problemi evidenti",\
    "Detectar a cada %d segundos e identificar problemas evidentes",\
    "Обнаружение каждые %d секунд и выявление явных проблем",\
    "Her %d saniyede bir tespit ederek belirgin sorunları tespit edin",\
    "%d秒ごとに検出し、明確な問題を特定する",\
    "%d초마다 검출하여 뚜렷한 문제를 발견합니다",\
};
...
const char *AIProModeTips[MAX_LANGUAGE] = { \
    "检测频率提高，可发现微小的问题",\
    "Increasing the detection frequency can reveal minor problems",\
    "Eine Erhöhung der Detektionsfrequenz kann kleinere Probleme aufdecken",\
    "Aumentar la frecuencia de detección puede revelar problemas menores",\
    "Augmenter la fréquence de détection peut révéler des problèmes mineurs",\
    "Aumentare la frequenza di rilevamento può rivelare problemi minori",\
    "Aumentar a frequência de detecção pode revelar problemas menores",\
    "Увеличение частоты обнаружения может выявить незначительные проблемы",\
    "Algılama frekansını artırmak, küçük sorunları ortaya çıkarabilir",\
    "検出頻度を上げると、微小な問題を発見できます",\
    "검출 빈도를 높이면 미세한 문제를 발견할 수 있습니다",\
};
...
const char *AIDetectionTips[MAX_LANGUAGE] = { \
    ...
    "AI 탐지를 통해 출력 과정 중 비정상적인 상황을 감지합니다.",\
};
...
const char *ExcludeObjects[MAX_LANGUAGE] = { \
    "排除对象",\
    "Exclude Objects",\
    "Ausschlussobjekt",\
    "Objeto de exclusión",\
...
const char *Layers[MAX_LANGUAGE] = { \
    "层数",\
    "层数",\
    "层数",\
    "层数",\
...
const char *DeviceBindTipsMsg[MAX_LANGUAGE] = { \
    "请使用创想云APP扫码绑定设备",\
    "Please scan the code to bind your device to the Creality Cloud",\
    "Bitte scannen Sie den Code, um Ihr Gerät mit der Creality Cloud zu verbinden",\
    "Escanee el código para vincular el dispositivo a Creality Cloud",\
    "Veuillez scanner le code pour relier votre appareil au Creality Cloud",\
    "Eseguire la scansione del codice per associare il dispositivo utilizzando Creality Cloud",\
    "Por favor, leia o código para vincular seu dispositivo à Creality Cloud",\
    "Сканируйте код, чтобы привязать устройство к Creality Cloud",\
    "Cihazınızı Creality Cloud'a bağlamak için lütfen kodu tarayın",\
    "Creality Cloudアプリによるコードスキャンで機器にバインドしてください",\
    "Creality Cloud 앱을 실행한 후 아래 QR 코드를 스캔해 연결하세요",\
};
...
const char *RootInfoTips[MAX_LANGUAGE] = { \
    "Root账户信息",\
    "Root account information",\
    "Root-Kontoinformationen",\
    "Información de la cuenta de root",\
    "Informations du compte root",\
    "Informazioni dell'account root",\
    "Informações da conta raiz",\
    "Информация об учетной записи Root",\
    "Root hesap bilgileri",\
    "ルートアカウント情報",\
    "Root 계정 정보",\
};
...
const char *PasswordTooLong[MAX_LANGUAGE] = { \
    "密码长度过长", \
    "密码长度过长", \
    "密码长度过长", \
    "密码长度过长", \
...
const char *SelfTestTipsMsg[MAX_LANGUAGE] = { \
    "#6A707D 自检过程预计需要##42bdd8 11##6A707D 分钟左右#", \
    "#6A707D Self-check process is expected to take ##42bdd8  11##6A707D  minutes#", \
    "#6A707D Die Selbstprüfung wird voraussichtlich ##42bdd8  11##6A707D  Minuten dauern#", \
    "#6A707D Se espera que el proceso de prueba automática tarde ##42bdd8  11##6A707D  minutos#", \
    "#6A707D Le processus d'auto-test devrait durer environ ##42bdd8  11##6A707D  minutes#", \
    "#6A707D Il processo di autotest richiede circa ##42bdd8  11##6A707D  minuti#", \
    "#6A707D Espera-se que o processo de autoteste leve##42bdd8  11##6A707D  minutos#", \
    "#6A707D Процесс самопроверки займет около ##42bdd8  11##6A707D   минут#", \
    "#6A707D Otomatik test işleminin yaklaşık ##42bdd8  11##6A707D  dakika sürmesi bekleniyor#", \
    "#6A707D セルフテストの所要時間は##42bdd8 11##6A707D 分程度を予定しています#",\
    "#6A707D 자체검사 과정은 약 は##42bdd8 11##6A707D 분 소요됩니다#",\
}; 
...
const char *HeaterPurifyingTip[MAX_LANGUAGE] = { \
    ...
    "1.印刷用台紙を置いてください",\
    "1.베드가 부착되어있는지 확인해 주세요",\
};

const char *SelfTestTimeTip[MAX_LANGUAGE] = { \
    "2.自检过程预计需要11分钟左右",\
    "2.The self-check process is expected to take around 11 minutes",\
    "2.Der Selbstprüfungsablauf wird voraussichtlich etwa 11 Minuten dauern",\
    "2.El proceso de la prueba automática se espera que tarde unos 11 minutos",\
    "2.Le processus d'auto-test devrait durer environ 11 minutes",\
    "2.Il processo di autotest richiede circa 11 minuti",\
    "2.Espera-se que o processo de autoteste leve cerca de 11 minutos",\
    "2.Процесс самопроверки займет около 11 минут",\
    "2.Otomatik test işleminin yaklaşık 11 dakika sürmesi bekleniyor",\
    "2.セルフテストの所要時間は約11分程度を予定しています",\
    "2.자체검사 과정은 약 11분 소요됩니다",\
}; 
...
const char *AgreePrivacyPolicy[MAX_LANGUAGE] = { \
    ...
    "Ich habe diese Datenschutzerklärung gelesen und\n stimme ihr zu",\
    "He leído y acepto la presente Política de privacidad",\
    "J'ai lu et j'accepte cette politique de confidentialité",\
    "Ho letto e accetto la presente Informativa sulla privacy",\
    "Li e concordo com esta Política de Privacidade",\
    "Я прочитал(-а) и согласен(-на) с настоящей Политикой\nконфиденциальности",\
    "Bu Gizlilik Politikasını okudum ve kabul ediyorum",\
    "私はこちらのプライバシーポリシーに同意します。",\
    "개인정보처리방침을 읽고 동의합니다",\
}; 

const char *AgreeRootPolicy[MAX_LANGUAGE] = { \
    "我已阅读且理解root登录风险",\
    "I have read and understood the risks of root login",\
    "Ich habe die Risiken des Root-Logins gelesen und verstanden",\
    "He leído y comprendido los riesgos del inicio de sesión root",\
    "J'ai lu et compris les risques de la connexion root",\
    "Ho letto e compreso i rischi dell'accesso come root",\
    "Eu li e entendi os riscos do login root",\
    "Я прочитал(а) и понял(а) риски входа в систему от имени root",\
    "Kök girişinin risklerini okudum ve anladım",\
    "私はrootログインのリスクを読み、理解しました",\
    "Root 로그인할 리스크를 읽고 동의합니다.",\
}; 

const char *PrivacyPolicyTips[MAX_LANGUAGE] = { \
    "需您同意#42BDD8 《隐私政策》#才能使\n用该服务",\
    "Your consent to the #42BDD8 Privacy Policy# is required to use the service",\
...
const char *ResetUserData[MAX_LANGUAGE] = { \
    ...
    "Подтвердить сброс?",\
    "Sıfırlama işlemini onaylıyor musunuz?",\
    "出荷時設定に復元しますか？",\
    "정말 초기화하시겠습니까?",\
};
...
const char *Key104Content[MAX_LANGUAGE] = { \
    ...
    "출력 베드에 이물질을 감지되어 출력이 일시정지되었습니다.",\
};
...
const char *Key508Content[MAX_LANGUAGE] = { \
    ...
    "현재 스토리지 공간 사용량이 %s에 도달했습니다, 시간에 맞게 정리하십시오", \
};
...
const char *Key2521Content[MAX_LANGUAGE] = { \
    "调平传感器芯片异常或传感器损坏", 
    "Leveling Sensor Chip Anomaly or Sensor Damage", \
    "Nivellierungssensor-Chip-Anomalie oder Sensorschaden", \
    "Anomalía del Chip del Sensor de Nivelación o Daño del Sensor", \
    "Anomalie de la Puce du Capteur de Nivellement ou Dommage du Capteur", \
    "Anomalia del Chip del Sensore di Livellamento o Danno del Sensore", \
    "Anomalia do Chip do Sensor de Nivelamento ou Dano do Sensor", \
    "Аномалия чипа датчика выравнивания или повреждение датчика", \
    "Seviye Sensörü Çipi Anormallikleri veya Sensör Hasarı", \
    "レベリングセンサーチップの異常またはセンサーの損傷", \
    "레벨링 센서 칩 이상 또는 센서 손상", \
};

const char *Key2522Content[MAX_LANGUAGE] = { \
    "调平传感器数据传输异常", 
    "Level Sensor Data Transmission Anomaly", \
    "Anomalie bei der Datenübertragung des Niveausensors", \
    "Anomalía en la Transmisión de Datos del Sensor de Nivel", \
    "Anomalie de Transmission de Données du Capteur de Niveau", \
    "Anomalia nella Trasmissione dei Dati del Sensore di Livello", \
    "Anomalia na Transmissão de Dados do Sensor de Nível", \
    "Аномалия Передачи Данных Датчика Уровня", \
    "Seviye Sensörü Veri İletim Anomalisi", \
    "レベルセンサーデータ伝送異常",\
    "레벨 센서 데이터 전송 이상",\
};
...
const char *Key2524Content[MAX_LANGUAGE] = { \
    "调平传感器芯片异常，传感器损坏或受环境影响", 
    "Leveling sensor chip abnormality, sensor damage or environmental impact.",\
    "Anomalie des Nivelliersensorschips, Sensorschaden oder Umwelteinfluss.",\
    "Anomalía del chip del sensor de nivelación, daño del sensor o impacto ambiental.",\
    "Anomalie de la puce du capteur de nivellement, dommages au capteur ou impact environnemental.",\
    "Anomalia del chip del sensore di livellamento, danno del sensore o impatto ambientale.",\
    "Anormalidade do chip do sensor de nivelamento, dano do sensor ou impacto ambiental.",\
    "Аномалия чипа датчика выравнивания, повреждение датчика или воздействие окружающей среды.",\
    "Düzleştirme sensörü çipi anormalliği, sensör hasarı veya çevresel etki.",\
    "レベリングセンサーチップの異常、センサーの損傷、または環境の影響。",\
    "레벨링 센서 칩 이상, 센서 손상 또는 환경 영향.",\
};
