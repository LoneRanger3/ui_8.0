...
static void hidden_file_controls(dirent_info_part_t *part);
static void hidden_dir_controls(dirent_info_part_t *part);
static void display_file_controls(dirent_info_part_t *part);
static void display_dir_controls(dirent_info_part_t *part);
...
dirent_info_part_t *create_custom_dirent_info_part(lv_obj_t *parent, int16_t pos_x, int16_t pos_y, void (*part_event_callback)(lv_event_t *))
{
    ...
    lv_obj_set_width(ui_PanelFileInfo, 158);
    lv_obj_set_height(ui_PanelFileInfo, 190);
    ...
    lv_obj_set_style_bg_color(ui_PanelFileInfo, lv_color_hex(0x0E0E0E), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_border_side(ui_PanelFileInfo, LV_BORDER_SIDE_FULL, LV_PART_MAIN | LV_STATE_PRESSED);
    lv_obj_set_style_border_color(ui_PanelFileInfo, lv_color_hex(0x4190A2),
                                  LV_PART_MAIN | LV_STATE_PRESSED);
    lv_obj_set_style_border_opa(ui_PanelFileInfo, 255, LV_PART_MAIN | LV_STATE_PRESSED);
    lv_obj_set_style_border_width(ui_PanelFileInfo, 2, LV_PART_MAIN | LV_STATE_PRESSED);
    ...
    lv_obj_set_width(ui_ImageThumbnail, 96);
    lv_obj_set_height(ui_ImageThumbnail, 96);
    lv_obj_set_x(ui_ImageThumbnail, 9);
    lv_obj_set_y(ui_ImageThumbnail, -10);
    ...
    lv_obj_set_width(ui_LabelFileName, 145);
    //lv_obj_set_height(ui_LabelFileName, LV_SIZE_CONTENT);    /// 34
    lv_obj_set_height(ui_LabelFileName, 60);    /// 34
    lv_obj_set_x(ui_LabelFileName, -15);
    lv_obj_set_y(ui_LabelFileName, 90);
    ...
    lv_obj_set_style_text_font(ui_LabelFileName, lv_font18.font, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_x(ui_ImageTime, -20);
    lv_obj_set_y(ui_ImageTime, 137);
    ...
    lv_obj_set_width(ui_ImageMaterial, 22);
    lv_obj_set_height(ui_ImageMaterial, 22);
    lv_obj_set_x(ui_ImageMaterial, 61);
    lv_obj_set_y(ui_ImageMaterial, 137);
    ...
    lv_obj_set_width(ui_LabelTime, 65);
    lv_obj_set_height(ui_LabelTime, LV_SIZE_CONTENT);    /// 32
    lv_obj_set_x(ui_LabelTime, 2);
    lv_obj_set_y(ui_LabelTime, 139);
    ...
    lv_obj_set_style_text_font(ui_LabelTime, lv_font14.font, LV_PART_MAIN | LV_STATE_DEFAULT);
    part->label_totle_time = ui_LabelTime;

    lv_obj_t * ui_LabelMaterial = lv_label_create(ui_PanelFileInfo);
    lv_obj_set_width(ui_LabelMaterial, 65);
    lv_obj_set_height(ui_LabelMaterial, LV_SIZE_CONTENT);    /// 32
    lv_obj_set_x(ui_LabelMaterial, 84);
    lv_obj_set_y(ui_LabelMaterial, 139);
    ...
    lv_obj_set_style_text_font(ui_LabelMaterial, lv_font14.font, LV_PART_MAIN | LV_STATE_DEFAULT);
    part->label_mater_used = ui_LabelMaterial;

    /*****************create dir info controls**********************/
    lv_obj_t *ui_ImageDir = lv_img_create(ui_PanelFileInfo);
    lv_img_set_src(ui_ImageDir, &ui_img_dirmini_png);
    lv_obj_set_width(ui_ImageDir, 64);
    lv_obj_set_height(ui_ImageDir, 50);
    ...
    lv_obj_set_width(ui_LabelDirCreateTime, 150);
    lv_obj_set_height(ui_LabelDirCreateTime, LV_SIZE_CONTENT);    /// 34
    lv_obj_set_x(ui_LabelDirCreateTime, -15);
    lv_obj_set_y(ui_LabelDirCreateTime, 139);
                        ...
                        uint32_t mater_used, uint32_t dirent_sort_index, bool local_disk, char *filament_type, int nozzle_temp, int bed_temp)
{
    ...
    else
    {
        ...
        if ( ch )
            *ch = '\0';
        strncpy(part->dirent_name, file_name, sizeof(part->dirent_name) - 1);
    ...
    part->is_dir = false;
    // part->filament_type = filament_type;
    strcpy(part->filament_type, filament_type);
    part->nozzle_temp = nozzle_temp;
    part->bed_temp = bed_temp;

    if ( part->label_totle_time )
    {
        char buff[20] = {0};
        uint32_t time = totle_time / 60;  // trans to minter
        sprintf(buff, "%0dh%02dm", time / 60, time % 60);
        lv_label_set_text(part->label_totle_time, buff);
        part->totle_time = totle_time;
    }

    if ( part->label_mater_used )
    {
        char buff[20] = {0};
        uint16_t meter = mater_used / 1000;
        if ( meter >= 100 )
        {
        ...
        {
            uint8_t cmeter = mater_used % 100;
            float fmeter = mater_used / 1000.0f;

            sprintf(buff, "%.1fm", fmeter + 0.05);
        }
        
        lv_label_set_text(part->label_mater_used, buff);
        part->mater_used = mater_used;
    }

    if ( part->img_file_png )
    {
        if ( mode_thumbnail && (strlen(mode_thumbnail) > 0) )
        {
            char buff[258] = {0};
            snprintf(buff, 258, "%s:%s", PNG_DRV_LETTER, mode_thumbnail);
