...
static void ui_event_PanelAddSpeed(lv_event_t * e);
static void ui_event_PanelSubSpeed(lv_event_t * e);
static void ui_event_PanelAddFlow(lv_event_t * e);
static void ui_event_PanelSubFlow(lv_event_t * e);
...
static void PrintSpeedAddClicked(lv_event_t * e);
static void PrintSpeedSubClicked(lv_event_t * e);
static void PrintFlowAddClicked(lv_event_t * e);
static void PrintFlowSubClicked(lv_event_t * e);
...
void ui_event_PanelAddSpeed(lv_event_t * e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t * target = lv_event_get_target(e);
    if(event_code == LV_EVENT_CLICKED) {
        PrintSpeedAddClicked(e);
    }
}
void ui_event_PanelSubSpeed(lv_event_t * e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t * target = lv_event_get_target(e);
    if(event_code == LV_EVENT_CLICKED) {
        PrintSpeedSubClicked(e);
    }
}
void ui_event_PanelAddFlow(lv_event_t * e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t * target = lv_event_get_target(e);
    if(event_code == LV_EVENT_CLICKED) {
        PrintFlowAddClicked(e);
    }
}
void ui_event_PanelSubFlow(lv_event_t * e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    lv_obj_t * target = lv_event_get_target(e);
    if(event_code == LV_EVENT_CLICKED) {
        PrintFlowSubClicked(e);
    }
}
...
/*******************增加静音模式开关***********************/
    ...
    lv_label_set_text(ui_LabelOffsetTitle, MoveDis[language_index]);
    ...
    lv_label_set_text(ui_LabelPrintSpeedAndFlowTitle, UnitSetting[language_index]);
...
void PrintSpeedAddClicked(lv_event_t * e)
{
	// Your code here
    if ( !is_lastest_speed_percent )
    {
        create_custom_reminderbox(SpeedTipsMsg[get_cur_language_index()], false);
        set_custom_reminderbox_display_timeout(3, NULL);
        return;
    }

    speed_percent += PRINT_SPEED_INCREASE_UNIT; //get_percent_unit_value();
    send_set_print_speed_cmd(speed_percent);
}

void PrintSpeedSubClicked(lv_event_t * e)
{
	// Your code here
    if ( !is_lastest_speed_percent )
    {
...
void PrintFlowAddClicked(lv_event_t * e)
{
	// Your code here
    if ( !is_lastest_flow_percent )
    {
        create_custom_reminderbox(FlowTipsMsg[get_cur_language_index()], false);
        set_custom_reminderbox_display_timeout(3, NULL);
        return;
    }

    flow_percent += PRINT_FLOW_INCREASE_UNIT;//get_percent_unit_value();
    send_set_print_flow_cmd(flow_percent);
}

void PrintFlowSubClicked(lv_event_t * e)
{
	// Your code here
    if ( !is_lastest_flow_percent )
    {
