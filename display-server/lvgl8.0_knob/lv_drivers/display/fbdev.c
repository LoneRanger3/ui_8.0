...
char *screenfbp[2] = {NULL, NULL};

#ifdef SYSTEM_PLATFORM_INGENIC
static unsigned long power_on = 0;
#endif // SYSTEM_PLATFORM_INGENIC
 ...
 **********************/
#ifdef SYSTEM_PLATFORM_INGENIC
#define CMD_read_power_on _IOWR('i', 84, unsigned long)
#endif // SYSTEM_PLATFORM_INGENIC
...
#ifdef USE_FBDEV_DOUBLE_BUFFER
    fbnum = (uint32_t)(screensize / (finfo.line_length * vinfo.yres));
    if (fbnum > 1) {
        printf("Turn on double buffering.\n");

        screenfbp[0] = fbp;
        screenfbp[1] = fbp + finfo.line_length * vinfo.yres;
        
        if (vinfo.yoffset == 0) {
            fbindex = !fbindex;
            fbp = screenfbp[fbindex];
        }
    }
#else
...
void fbdev_flush(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
...
#ifdef USE_FBDEV_DOUBLE_BUFFER
            if (fbnum > 1)
                location = act_x1 + y * finfo.line_length;
            else
#endif // USE_FBDEV_DOUBLE_BUFFER
    ...
    if (fbnum > 1 && lv_disp_flush_is_last(drv)) {
        vinfo.yoffset = fbindex * vinfo.yres;
#ifdef SYSTEM_PLATFORM_INGENIC
        if (ioctl(fbfd, CMD_read_power_on, &power_on) == 0) {
            if (power_on == 1) {
#endif // SYSTEM_PLATFORM_INGENIC
                ...
                perror("Error: CMD_read_power_on fail");
        }
#endif // SYSTEM_PLATFORM_INGENIC

        memcpy(screenfbp[!fbindex], screenfbp[fbindex], finfo.line_length * vinfo.yres);
        fbindex = !fbindex;
        fbp = screenfbp[fbindex];
    }
#endif // USE_FBDEV_DOUBLE_BUFFER
