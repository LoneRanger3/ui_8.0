 ...
 * @LastEditTime: 2023-11-03 03:50:47
 */
#include <sys/msg.h>
#include <sys/types.h>
#include <sys/time.h>
...
static void DevSendDeviceAllInfoToDis(MsgHeadBuf_t *head)
{
    ...
    else if (strcmp(systemComfig.model_str, "PF-05") == 0) strcat(tempModelStr, "PF05");
...
static void  ScreenDisplayControl(int sw)
{
    ...
    //将原来的息屏方式改为修改背光PWM的方式，息屏设备0
    if (sw)
    { 
        system_state.light_value = light_value_tmp;
        CrLogI("system_state.light_value = %d", system_state.light_value);
        if (system_state.light_value)
        {
            sprintf(resBuf, "/usr/bin/brightness.sh %d", system_state.light_value);
            CrLogI("%s", resBuf);
            printf("%s\n", resBuf);
            system(resBuf); 
        }
    ...
    else
    {
        if (system_state.light_value)
        {
            light_value_tmp = system_state.light_value;
            CrLogI("/usr/bin/brightness.sh 0");
            system("/usr/bin/brightness.sh 0");
        }
...
static void DetectStartupScreenSaver(void)
{
    ...
    if (system_state.ScreenSaverCount < startupTime + 10) {
        if (++system_state.ScreenSaverCount == startupTime)
        {
        ...
        switch (event->mask)
        {
            ...
            case IN_MODIFY:
            {
                if (strstr(changePath, GetUserLocalGcodeInfoDirPtah())) {
                    // 同步本地文件
                    CrLogI("test sync local file info");
                    // printf("test sync local file info\n");
                    MonitorLocalDirectory(GetUserPrinterGcodesFilePath());
                }
                else if (strstr(changePath, GetUserUdiskGcodeInfoDirPtah())) {
                    // 同步U盘文件
                    CrLogI("test sync udisk file info");
                    MonitorUdiskDirectory(printRunState.uDiskPath);
                    //printf("printRunState.uDiskPath:%s\n", printRunState.uDiskPath);
                }
                break;
            }
...
/**
 * @description:  获取状态 :1. 从 master-server/Display  状态同步到printRunState systemComfig等结构体中获取 2. 发送命令到 master-server/Display 请求同步
 * @param  {*}
 * @return {*}
...
int GetPrinterDataFromManager(unsigned char flag, unsigned char dataLen, char *data)
{
    ...
    switch (flag)
    {
        ...
        case UI_GET_CURRENT_WIFI_INFO:
        {
            ...
            CrLogI("UI_GET_CURRENT_WIFI_INFO, end");
            break;
        }
...
static void UserExportLogToUdisk(char *path)
{
    ...
    if (access(printRunState.uDiskPath, F_OK) != 0) {
        CrLogW("U disk no exist \n");
        //DevSendCurExportLogStateToDis(EXPORT_LOG_UDISK_NO_EXIT, 0);
        CreateInsertUdiskTipDialog(3000);
        return;
    }
...
static void *UserCopyGcodeThread(void *arg)
{
    ...
    //     file_copyFileToDir(selectFileCollection[i], copyFileTargetPath);
    // }
    char tempPath[256] = {0};
    strcat(tempPath, copyFileTargetPath);
    strcat(tempPath, strrchr(ActionGcodes.filePath, '/'));
    strcat(tempPath, ".temp");
    CrLogI("tempPath = %s", tempPath);
    if (strlen(ActionGcodes.filePath) > 0)
        file_copyFileToDir(ActionGcodes.filePath, tempPath);
    

    // 获取源文件MD5
    char sourceFileMd5[256] = {0};
    getFileMD5(ActionGcodes.filePath, sourceFileMd5);
    // 获取新文件MD5
    ...
    // 重命名文件
        ...
        strcat(newFilePath, strrchr(ActionGcodes.filePath, '/'));
    ...
    // 应答
   // snprintf(sendBuff, 100, "12\t%d", 100);
...
static void *UserDeleteGcodeThread(void *arg)
{
    ...
    // }
    if (strlen(ActionGcodes.filePath) > 0)
        remove(ActionGcodes.filePath);  
    CrLogI("ActionGcodes.filePath :%s\n", ActionGcodes.filePath);
    curSelectFileCount = 0;
    selectFileCollectionNum = 0;
    bzero(selectFileCollection, sizeof(selectFileCollection));
...
/**
 * @description: 设置数据后同步发送到 master-server/Display 
 * @param  {*}
 * @return {*}
 * @param {unsigned char} flag
 * @param {unsigned char} len
