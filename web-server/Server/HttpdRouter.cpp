 ...
 * @ Modified time: 2023-03-07 19:56:02
 * @ Description:
 */
...
void Router::Register(hv::HttpService& router) {
    ...
    // curl -v -O http://ip:port/downloads/video/123456789.mp4
    router.GET("/downloads/video/{filename}", Handler::sendVideoFile);
    // curl -v -O http://ip:port/downloads/gcode/123456789.gcode
    router.GET("/downloads/gcode/{filename}", Handler::sendGcodeFile);

    // GetUserPrinterGcodesFilePath
    // SSE: Server Send Events
    // @test html/EventSource.html EventSource.onmessage
    router.GET("/sse", Handler::sse);

    // curl -v http://ip:port/info
