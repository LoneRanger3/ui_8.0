 ...
 * @ Modified time: 2023-05-08 14:34:04
 * @ Description:
 */
...
int Handler::upload(const HttpContextPtr& ctx) {
    ...
    char gcodePath[DEF_PATH_LEN] = {0};
    snprintf(gcodePath, DEF_PATH_LEN, "%s/", GetUserLocalSdCardPtah());
    std::string save_path = gcodePath;
...
int Handler::recvLargeFile(const HttpContextPtr& ctx, http_parser_state state, const char* data, size_t size) {
    recvlargefile *revf = nullptr;
    if(ctx->userdata == NULL)
    {
        revf = new recvlargefile();
        ctx->userdata = revf;
    }else{
        revf = (recvlargefile*)ctx->userdata;
    }
    //CrLogI("recvLargeFile state=%d handle 0x%x\n", (int)state,revf);
    int status_code = HTTP_STATUS_UNFINISHED;
    switch (state) {
        case HP_HEADERS_COMPLETE:
            {
                if (ctx->is(MULTIPART_FORM_DATA)){
                    auto iter = ctx->headers().find("Content-Type");
                    if (iter == ctx->headers().end()) {
                        ctx->close();
                        return HTTP_STATUS_BAD_REQUEST;
                    }
                    ...
                    revf->strBoundary = boundary;
                    revf->strBoundary = hv::trim_pairs(revf->strBoundary, "\"\"\'\'");
                }
                CrLogI("start upload file from http");
                CrLogI("client_addr.ip = %s", ctx->ip().c_str());
                CrLogI("client_addr.port = %d", ctx->port());
                
                char gcodePath[DEF_PATH_LEN] = {0};
                snprintf(gcodePath, DEF_PATH_LEN, "%s/", GetUserPrinterGcodesFilePath());
                revf->save_path = gcodePath;
                revf->filename = ctx->param("filename", "unnamed.txt");
                revf->filepath = revf->save_path + urlDecode(revf->filename.c_str());
                revf->tmppath = revf->filepath;
                size_t pos = revf->tmppath.find(".gcode");
                if (pos != std::string::npos) {
                    revf->tmppath.replace(pos + 1, 5, "tmp"+to_string((int)revf));
                }

                revf->file = new HFile;
                CrLogI("filepath = %s", revf->tmppath.c_str());
            }
            break;
        case HP_BODY:
            {
                if (revf->file && data && size) {
                    if(strstr(data, revf->strBoundary.c_str()) && strstr(data, "Content-Disposition") && strstr(data, "Content-Type")){
                        std::string datastr = data;
                        if (revf->file->open(revf->tmppath.c_str(), "wb") != 0) {
                            ctx->close();
                            return HTTP_STATUS_INTERNAL_SERVER_ERROR;
                        }
                        uint len = datastr.find("\r\n\r\n") + strlen("\r\n\r\n");
                        if(revf->file->write(data + len, size-len) != (size-len)){
                            ctx->close();
                            remove(revf->file->filepath);
                            system("sync");
                            return HTTP_STATUS_INTERNAL_SERVER_ERROR;
                        }                      
                    }else if (strstr(data, revf->strBoundary.c_str()) && !(strstr(data, "Content-Disposition")) && !(strstr(data, "Content-Type"))){
                        uint len = 8+ revf->strBoundary.length();
                        if(revf->file->write(data, size-len) != (size-len)){
                            ctx->close();
                            remove(revf->file->filepath);
                            system("sync");
                            return HTTP_STATUS_INTERNAL_SERVER_ERROR;                            
                        ...
                        if (revf->file->write(data, size) != size) {
                            ctx->close();
                            remove(revf->file->filepath);
                            system("sync");
                            return HTTP_STATUS_INTERNAL_SERVER_ERROR;
        ...
        case HP_MESSAGE_COMPLETE:
            {
                std::string renamePath = revf->file->filepath;
                string tail = ".tmp"+to_string((int)revf);
                size_t pos = renamePath.find(tail);
                if (pos != std::string::npos) {
                    renamePath.replace(pos + 1, tail.length()-1, "gcode");
                }

                CrLogI("renamePath = %s\n", renamePath.c_str());
                rename(revf->file->filepath, renamePath.c_str());
                system("sync");
...
int Handler::sendVideoFile(const HttpContextPtr& ctx)
{
    int pos = ctx->request->Path().rfind("/");
    std::string filepath = GetUserDelayImageVideoPtah();
    if(pos!=std::string::npos)
    {
        filepath += std::string(ctx->request->Path(),pos);
    }
    return sendLargeFile(ctx,filepath);
}

int Handler::sendGcodeFile(const HttpContextPtr& ctx)
{
    int pos = ctx->request->Path().rfind("/");
    std::string filepath = GetUserPrinterGcodesFilePath();
    if(pos!=std::string::npos)
    {
        filepath += std::string(ctx->request->Path(),pos);
    }
...
int Handler::sendLargeFile(const HttpContextPtr& ctx,string &filepath) {
    std::thread([ctx,filepath](){
        ctx->writer->Begin();
        CrLogI("sendLargeFile PATH=%s\n", filepath.c_str());
        HFile file;
...
int Handler::info(HttpRequest* req, HttpResponse* resp) {
    json_object *iotConfig = json_object_from_file(GetUserSystemConfigFilePath());
    std::string modelStr = "";
    std::string macStr = "";
    if (iotConfig){
        json_object *devInfoObj = NULL, *modelObj = NULL, *macObj = NULL;
    ...
    resp->json["model"] = modelStr.substr(modelStr.find('-') + 1);
    resp->json["mac"] = macStr;
    return 200;
}
